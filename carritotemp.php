...
if ( isset($_SESSION['carrito']) || isset($_POST['nombre'])){
    ...
    # se descuenta en compra /si no regresa
...
$resultado = mysql_query("SELECT cant_pro FROM producto WHERE nom_pro='".$nombre."'");
$fila = mysql_fetch_row($resultado);


          #echo '<script> alert("'.$fila[0].'")</script>';   //pa ver cuantos compara
          if($fila[0]>=1){
            // echo"<script>alert('entro')</script>";     //para saber si entro o se va al else
             ...
             $query2 = "UPDATE producto SET cant_pro=cant_pro-".$cantidad." where nom_pro='".$nombre."'";
             mysql_query($clavepro);  
             mysql_query($query);  
             mysql_query($query2);  
              echo"<script>alert('Has insertado un producto en tu carrito')</script>";
              ...
              echo"<script>alert('no hay productos en existencia')</script>";
              echo "<script type='text/javascript'>
                        history.go(-1);
                    </script>";
          }
          
    # se descuenta en compra
...
for($i=0;$i<=count($compras)-1;$i++){
  ...
  <tr align="center">
    ...
    <?php echo $compras[$i]['cantidad'] * $compras[$i]['precio'];?>  <!-- eliminar.php -->
    <span id="toolTipBox" width="200"></span>
       <input name="id2" type="hidden" id="id2" value="<?php echo $i;?>"> 
               <input type="image" name="imageField" id="imageField" src="imagenes/eliminar.png" onmouseover="toolTip('Presione para eliminar su pedido',this)"></form></td>
  </tr>
  <?php
...
alert(.$query."Has ingresado un nuevo producto al carrito");
